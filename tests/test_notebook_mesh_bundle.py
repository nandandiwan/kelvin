...
def test_generic_notebook_bundle_remains_supported(tmp_path):
    context = create_context()
    path = ROOT / "data/sky130/gds/sky130_fd_sc_hd__inv_1.gds"
    ...
    assert saved["top_cell"] == "sky130_fd_sc_hd__inv_1"
    assert "source_mapping_sha256" not in saved
    assert saved["mesh_regions_sha256"] == stats["mesh_regions_sha256"]
    assert paths["manifest"].is_file()


def test_notebook_configuration_case_switch_resets_sram_only_state(monkeypatch):
    namespace = {}
